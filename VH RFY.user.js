// ==UserScript==
// @name       Play sound when unhidden vine RFY item detected
// @match      https://www.amazon.co.uk/vine/vine-items?queue=potluck
// @grant      none
// ==/UserScript==

// Quack!!!
var alertSound = new Audio ("data:audio/mp3;base64,");

// between 3 and 8 seconds
var reloadInterval = ((Math.floor(Math.random() * 5)) + 3) * 1000;
console.log('Delay: ' + reloadInterval);

var refreshTimeout;

if (! document.hasFocus()) {
  play();
}

window.addEventListener("blur", play);
window.addEventListener("focus", pause);

function play() {
    console.log('Tab is inactive. Setting timeout...');
    refreshTimeout = setTimeout(refreshMe, reloadInterval);
}

function pause() {
    console.log('Tab is active. Cancelling refresh...');
    clearTimeout(refreshTimeout);
}


function refreshMe() {
    if (! document.getElementById('vh-tabs')) {
        console.log('Stopping!');
        // We should stop running, might be a CAPTCHA
        window.removeEventListener("blur", play);
        window.removeEventListener("focus", pause);
        // Quack!!!
        alertSound.play ();
        return;
    }
    var itemCount = document.getElementById('vvp-items-grid').getElementsByClassName('vvp-item-tile').length;
    if (itemCount) {
        // Must hide item and refresh page before we run again
        window.removeEventListener("blur", play);
        window.removeEventListener("focus", pause);
        // Quack!!!
        alertSound.play ();
    }
    else {
        console.log('Reloading!');
        location.reload();
    }
}
